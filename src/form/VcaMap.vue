    ...
    <div>
        ...
        <div v-if="isLoaded" class="vca-mapdata">
            ...
            <vca-input
                ...
                @input="updateMapsMarker"
                v-model="current.position.lng"
            />
            <vca-dropdown
                v-if="!disabled && !hideFields"
                :options="markerTypes"
                @input="updateMapsMarker"
                label=""
...
export default {
            ...
            default: function () {
                return [
                    ...
                    },
                    {
                        ...
                        label: "Info",
                        icon: "http://maps.google.com/mapfiles/kml/shapes/info.png",
                        value: "info",
                    },
                    {
                        title: "Camping",
                        subtitle: "Unser Campingplatz",
                        label: "Camping",
    ...
    methods: {
        ...
        centerMarker() {
            this.gotoCenter(
                new window.google.maps.LatLng(
                    Number(this.current.position.lat),
                    Number(this.current.position.lng)
                )
            );
        },
        ...
        addMarker(markerArg) {
            ...
            var index = this.markerTypes.findIndex(
                (el) => el.value == markerArg.type
            );
        ...
        updateMapsMarker() {
            ...
            var index = this.mapsMarker.findIndex(
                (el) => el.id == this.current.id
            );
            if (index == -1) {
                return;
            }

            this.mapsMarker[index].setPosition(
                new window.google.maps.LatLng(
                    Number(this.current.position.lat),
                    Number(this.current.position.lng)
                )
            );
            this.mapsMarker[index].setTitle(
            ...
            var iconIndex = this.markerTypes.findIndex(
                (el) => el.value == this.current.type[0].value
            );
        ...
        updateMarkerList() {
            ...
            var index = this.markerList.findIndex(
                (el) => el.id == this.current.id
            );
            if (index == -1) {
                return;
            ...
            this.markerList[index].position = {
                lat: Number(this.current.position.lat),
                lng: Number(this.current.position.lng),
            };
        ...
        mapCallback() {
            ...
            if (this.value.length > 0) {
                this.currentCenter = {
                    lat: Number(this.value[0].position.lat),
                    lng: Number(this.value[0].position.lng),
                };
                this.zoom = 12;
            }
