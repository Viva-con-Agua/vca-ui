<template>
  <div class="info-box">
    <div class="info-icon">
      <img @mouseenter="hovered = true, isRightOfViewPort($el), isBottomOfViewPort($el)" @mouseleave="hovered = false"
           src="data:image/png;base64,">
    </div>
    <div class="info-text-wrap">
    <div name="infotext"
         v-bind:class="{'hide' : !hovered, 'info-text-left' : left, 'info-text-right' : !left, 'info-text-top' : top, 'info-text-bottom' : !top, 'info-text' : true}">
      {{ infoText }}
    </div>
    </div>
  </div>
</template>

<script>
export default {
  name: "VcaInfoBox",
  props: ["infoText"],
  data() {
    return {
      hovered: false,
      left: false,
      top: false
    }
  },
  methods: {
    isRightOfViewPort(element) {
      if (this.calculatedLeft) {
        return
      }
      var el = element.lastElementChild.children[0];
      var rect = el.getBoundingClientRect();

      if(rect.right > (window.innerWidth || document.documentElement.clientWidth)){
        this.left = true;
      }
    },
    isBottomOfViewPort(element) {

      if (this.calculatedTop) {
        return
      }
      var el = element.lastElementChild;
      var rect = el.getBoundingClientRect();

      this.top = rect.bottom > (window.innerHeight || document.documentElement.clientHeight);
    }

  }
}
</script>

<style lang="scss">

.info-box {
  width: 100%;
  height: auto;
  text-decoration: none;
  position: relative;
  overflow: visible;


  .info-icon {
    position: relative;
    z-index: +3;

    img {
      width: 25px;
    }
  }

  .info-text-wrap{

    position: absolute;
    top: 0;
    width: 10000%; //take as much space as one needs not restricted by available space
    max-width: 400px;

    .info-text {
      position: relative;
      overflow: visible;
      width: 10000%; //take as much space as one needs not restricted by available space
      max-width: 400px;
      z-index: +5;
      text-align: justify;

      box-shadow: 0 4px 8px 2px rgba(0, 0, 0, 0.2);
      transition: 0.3s;
      padding: 1.0em;
      background: #ffffff;
    }


    .info-text-right {
      left: 35px;
    }

    .info-text-left {
      right: calc(100% + 45px);
    }

    .info-text-top {
      bottom: 0px;
    }

    .info-text-bottom {
      top: 0px;
    }

    .hide {
      visibility: hidden;
    }
  }



}


</style>
